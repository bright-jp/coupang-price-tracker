"""
Coupang Price Tracker - Bright Data Web Scraper API
----------------------------------------------------
Track prices, availability, and product data from Coupang.

Docs  : https://docs.brightdata.jp/scraping-automation/web-scraper-api/overview
Info  : https://brightdata.jp/products/insights/price-tracker/coupang
"""
...
# Search for "Coupang" to find the matching Web Scraper (format: gd_xxxxxxxxxx).
DATASET_ID = os.getenv("BRIGHTDATA_DATASET_ID", "")
if not DATASET_ID:
    raise EnvironmentError(
        "BRIGHTDATA_DATASET_ID is not set.\n"
        "1. Navigate to: https://brightdata.jp/cp/scrapers\n"
        "2. Search for 'Coupang' and copy the Web Scraper ID.\n"
...
def track_prices(urls: list[str], **kwargs) -> list[dict]:
    """
    Collect price data for a list of Coupang product URLs.
...
def discover_by_keyword(keyword: str, limit: int = 50, **kwargs) -> list[dict]:
    """
    Discover Coupang products by keyword search.
...
def discover_by_category(category_url: str, limit: int = 100, **kwargs) -> list[dict]:
    """
    Discover all products from a Coupang category page.

    Example:
        results = discover_by_category("https://coupang.com/s?k=headphones", limit=200)
    """
...
if __name__ == "__main__":
    ...
    sample_urls = [
        "https://www.coupang.com/product/sample-item-123456",
        # Add more Coupang product URLs here
    ]
    results = track_prices(sample_urls)

    output_file = "coupang_prices.json"
    ...
    # cat_results = discover_by_category("https://coupang.com/category/sample", limit=50)
